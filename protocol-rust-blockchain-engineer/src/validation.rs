...
impl Validator {
    ...
    pub fn validate_block(&self, block: &Block, state: &StateDB) -> Result<()> {
...
mod tests {
    ...
    fn test_validate_block_empty_block() {
        ...
        Executor::new(state.clone()).execute_block(&block).unwrap();
        assert!(validator.validate_block(&block, &state).is_ok());
    }
}
